...
function drawPattern(dis) {
  ...
  for (let j = 0; j < bio.length; j++) {
    ...
    for (let i = 0; i < bioDis[j].length; i++) {
      ...
      pattern.fill(250, map(mouseY, 0, pattern.height/2, 100, 135));
...
function mousePressed() {
  ...
  if (d1 < 100) {
    location.reload();
    location.href = "https://samesamebutdifferent-12.netlify.app";
  }

  if (d2 < 100) {
    location.reload();
    location.href = "https://samesamebutdifferent-02.netlify.app";
  }
